...
const GROUPS: { heading: string; note: string; rows: [string, string][] }[] = [
  ...
  },
  {
    ...
    rows: [
      [
        "MODALITY_COMPARISON",
        "Nine rows as assessed on 4 August 2026, with what each acts on, predicted current, best precedent, what gates it, and whether it depends on the unresolved mechanism. Three cautions, and they matter more than the table does. Two of the nine rows are current care rather than routes, an implanted defibrillator and existing drugs. Four therapeutic routes that the routes page ranks are missing from this file entirely: prime editing, which is ranked second; chaperone and interacting-partner upregulation, added on 6 August after this table was built; and two more added later the same day, a compact surrogate sodium channel and raising the healthy copy's output by means other than redirecting discarded messages. And every figure in the predicted-current column was computed on the superseded rescaling withdrawn on the science page, so those percentages should not be reused, while the gates and precedents are unaffected. One row here is worth more than the rest of the file: the interaction and coupled-gating drug, which this table has carried since 4 August and which the project's own route inventory had no entry for until 6 August 2026. This file is half the evidence that the route was lost rather than dropped. The routes page, not this file, is the current list.",
      ],
      [
